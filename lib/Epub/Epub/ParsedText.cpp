...
void ParsedText::addVerticalToken(std::string token, const EpdFontFamily::Style fontStyle,
  ...
  // though the vertical renderer does not draw them yet. Keep the array in lockstep regardless:
  // once it is non-empty, a missing entry would shift every later word's ruby onto its neighbour.
  if (!rubyTexts.empty()) {
    rubyTexts.push_back("");
  }
}
