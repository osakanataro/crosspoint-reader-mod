...
class ParsedText {
  ...
  // TextBlocks do not carry any of this metadata. Not populated in vertical mode
  // (see addVerticalToken); vertical pages instead fall back to the parser's running
  // visibleTextOffset, which by then points at the end of the parsed paragraph, so their
  // recorded positions run ahead of the true one.
