...
bool Epub::parseContentOpf(BookMetadataCache::BookMetadata& bookMetadata, const bool writeSpineEntries) {
  ...
  // The named cover is an SVG wrapper rather than a picture. Follow the one <image> inside it,
  // the same step the chapter path already takes for a wrapper in the body; the cover BMP
  // generation below only understands JPEG and PNG, so without this the book gets no cover.
  ...
  CSS_DIAG("opf cover=%.40s", bookMetadata.coverItemHref.empty() ? "(none)" : bookMetadata.coverItemHref.c_str());
...
bool Epub::generateThumbBmp(int height) const {
  ...
  const auto coverImageHref = bookMetadataCache->coreMetadata.coverItemHref;
  CSS_DIAG("thumb href=%.38s", coverImageHref.empty() ? "(none)" : coverImageHref.c_str());
  if (coverImageHref.empty()) {
    ...
    // The zip read needs deflate's 32 KB window, which is the first thing to go missing on a
    // fragmented heap. Without this check the empty temp file goes on to the decoder and the
    // failure is reported as a bad JPEG.
    if (!readItemContentsToStream(coverImageHref, coverJpg, 1024)) {
      CSS_DIAG("thumb extract FAIL jpg");
      coverJpg.close();
      Storage.remove(coverJpgTempPath.c_str());
      return false;
    }
    ...
    if (!readItemContentsToStream(coverImageHref, coverPng, 1024)) {
      CSS_DIAG("thumb extract FAIL png");
      coverPng.close();
      Storage.remove(coverPngTempPath.c_str());
      return false;
    }
